	...
	for i, tt := range tests {
		...
		if tt.themed {
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-painter", func(t *testing.T) {
				...
				validateRadarChartRender(t, p, rp, tt.makeOptions(), tt.result, tt.crc32)
			})
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-options", func(t *testing.T) {
				...
				validateRadarChartRender(t, p, rp, opt, tt.result, tt.crc32)
			})
		} else {
			t.Run(strconv.Itoa(i)+"-"+tt.name, func(t *testing.T) {
				...
				validateRadarChartRender(t, p.Child(PainterPaddingOption(NewBoxEqual(20))),
					rp.Child(PainterPaddingOption(NewBoxEqual(20))), tt.makeOptions(), tt.result, tt.crc32)
			})
		}
	}
}

func validateRadarChartRender(t *testing.T, p, rp *Painter, opt RadarChartOption, expectedResult string, expectedCRC uint32) {
	t.Helper()
