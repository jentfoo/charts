	...
	for i, tt := range tests {
		...
		if !tt.themed {
			t.Run(strconv.Itoa(i)+"-"+tt.name, func(t *testing.T) {
				...
				validateScatterChartRender(t, p, rp, tt.makeOptions(), tt.result, tt.crc32)
			})
		} else {
			...
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-theme_opt", func(t *testing.T) {
				...
				validateScatterChartRender(t, p, rp, opt, tt.result, tt.crc32)
			})
		}
	}
}

func validateScatterChartRender(t *testing.T, p, rp *Painter, opt ScatterChartOption, expectedResult string, expectedCRC uint32) {
	t.Helper()
