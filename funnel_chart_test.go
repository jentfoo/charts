	...
	for i, tt := range tests {
		...
		if tt.themed {
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-painter", func(t *testing.T) {
				...
				validateFunnelChartRender(t, p, rp, tt.makeOptions(), tt.result, tt.pngCRC)
			})
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-options", func(t *testing.T) {
				...
				validateFunnelChartRender(t, p, rp, opt, tt.result, tt.pngCRC)
			})
		} else {
...
func validateFunnelChartRender(t *testing.T, p, rp *Painter, opt FunnelChartOption, expectedResult string, expectedCRC uint32) {
	t.Helper()
