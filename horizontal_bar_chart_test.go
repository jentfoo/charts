	...
	for i, tt := range tests {
		...
		if tt.themed {
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-painter", func(t *testing.T) {
				...
				validateHorizontalBarChartRender(t, p, rp, tt.makeOptions(), tt.result, tt.pngCRC)
			})
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-options", func(t *testing.T) {
				...
				validateHorizontalBarChartRender(t, p, rp, opt, tt.result, tt.pngCRC)
			})
		} else {
...
func validateHorizontalBarChartRender(t *testing.T, p, rp *Painter, opt HorizontalBarChartOption, expectedResult string, expectedCRC uint32) {
	t.Helper()
