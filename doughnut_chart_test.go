	...
	for i, tt := range tests {
		...
		if tt.themed {
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-painter", func(t *testing.T) {
				...
				validateDoughnutChartRender(t, p, rp, tt.makeOptions(), tt.result, tt.pngCRC)
			})
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-options", func(t *testing.T) {
				...
				validateDoughnutChartRender(t, p, rp, opt, tt.result, tt.pngCRC)
			})
		} else {
			t.Run(strconv.Itoa(i)+"-"+tt.name, func(t *testing.T) {
				...
				validateDoughnutChartRender(t, p.Child(PainterPaddingOption(NewBoxEqual(20))),
					rp.Child(PainterPaddingOption(NewBoxEqual(20))), tt.makeOptions(), tt.result, tt.pngCRC)
			})
		}
	}
}

func validateDoughnutChartRender(t *testing.T, p, rp *Painter, opt DoughnutChartOption, expectedResult string, expectedCRC uint32) {
	t.Helper()
	...
	err = rp.DoughnutChart(opt)
	require.NoError(t, err)
	rasterData, err := rp.Bytes()
	require.NoError(t, err)
